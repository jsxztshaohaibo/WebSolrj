...
<script type="text/JavaScript"> 
		$(document).ready(function(){
			...
			$("#doUpdateCJCommit").bind("click",checkForm);
			
			
			//赋值 
        	$("#kmgbm").val(${dto.kmgbm});
        	//$("#kmgbm option[value='" + ${dto.kmgbm} + "']").attr("selected", true);
			$("input[name='cjyq'][value='${dto.cjyq}']").attr("checked",true);
			if(${dto.cjyq} == 1){
				$("#zdf").attr("disabled",false);
		    	$("#zgf").attr("disabled",true);
		    	$("#zdf").val(${dto.zdf});
       ...
       function checkForm(){
    	   ...
    	   var data ={cjId:${dto.id} ,kmgbm:kmgbm,cjyq:cjyq,zdf:zdf,zgf:zgf};
    	   $.post("${ctx}/toudang/TouDangTemplateAction.a?doUpdateCJCommit",
    			  data,
    	   		  function(res){
    				  if(res.msg=="success"){
    					  alert("保存成功!");
					...
					<tr class="even">
						...
						<td>
							<div align="left" style="margin-left:20px">
								<div  style="line-height: 45px; height:45px;">最低分数：<input type="text" name="zdf" id="zdf" style="width:50px"/></div>
								<div  style="line-height: 45px; height:45px;">最高分数：<input type="text" name="zgf" id="zgf" style="width:50px"/></div>
							</div>
						</td>
					</tr>
				</table>
		...
		<div id="operationarea">
			<div id="operationcontent">
				<div align="center">
					...
					<input name="doUpdateCJCommit" type="button"  id="doUpdateCJCommit" value="  提交  " /> 
				</div>
			</div>
		</div>
</form>
